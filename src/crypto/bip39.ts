...
export function bytesToMnemonic(bytes: Uint8Array, wordCount: number = 12): string {
  ...
  const checksum = bytes[0] & ((1 << checksumBits) - 1);
  ...
  const words = indices.map((index: number) => BIP39_WORDLIST[index]);
  return words.join(' ');
}
...
export function mnemonicToBytes(mnemonic: string): Uint8Array {
  ...
  const indices = words.map((word: string) => {
  ...
  const bytes = new Uint8Array(Math.ceil(dataBits.length / 8));
  for (let i = 0; i < dataBits.length; i += 8) {
    let byte = 0;
    for (let j = 0; j < 8 && i + j < dataBits.length; j++) {
      byte |= (dataBits[i + j] ? 1 : 0) << (7 - j);
    }
...
export async function generateMnemonic(wordCount: number = 12): Promise<string> {
  const entropySize = wordCount === 12 ? 16 : 32;
  const entropy = randomBytes(entropySize);
  return bytesToMnemonic(entropy, wordCount);
}
...
export async function deriveEd25519Keypair(seed: Uint8Array): Promise<{
  ...
  const privateKeyBits = await crypto.subtle.deriveBits(
    {
      ...
      hash: 'SHA-512',
      salt: new Uint8Array([]),
      info: new TextEncoder().encode('ed25519 private key'),
    },
    baseKey,
    256
  );
